...
class Model {
  ...
  generateTbody(year, month) {
    ...
    let pastMonthDays = this.pastMonthDays(
      daysInYearArr,
      year,
      month,
      pastMonth
    );
    let nextMonthDays = this.nextMonthDays(
      daysInYearArr,
      pastMonthDays,
      monthDays,
      year,
  ...
  intermediateSelectFunc(calendarDays, year, month, tbody) {
    ...
    calendarDays.forEach((el, idx) => {
      dayElements[idx].classList.remove(
        "mid-selected",
        "left",
        "right",
        "selected"
  ...
  pastMonthDays(daysInYearArr, year, month, pastMonth) {
    ...
    !firstMonthWeekDay
      ? (firstMonthWeekDay = 6)
      : (firstMonthWeekDay = firstMonthWeekDay - 1);
    ...
    for (let i = 0; i < firstMonthWeekDay; i++) {
      pastMonthDays.push(
        this.elObject(year, pastMonth, daysInYearArr[pastMonth].length - i)
      );
    }
  ...
  nextMonthDays(daysInYearArr, pastMonthDays, monthDays, year, month) {
    ...
    nextMonthDays = nextMonthDays.slice(
      0,
      42 - (pastMonthDays.length + monthDays.length)
    );
    ...
    $("td").each((index, el) => {
      ...
      $(el).on("click", (e) => {
        ...
        if ($(".input-dates").length === 1) {
          $(".input-dates").attr(
            "placeholder",
            `${selectedObj.day} ${monthsNames[selectedObj.month]}`
          );
          ...
          if (this.selectedArr.length === 2) {
            ...
            $(".input-dates")[0].setAttribute(
              "placeholder",
              `${selectedSortedArr[0].day} ${
                monthsNames[selectedSortedArr[0].month]
              } - ${selectedSortedArr[1].day} ${
                monthsNames[selectedSortedArr[1].month]
              }`
            );
          }
  ...
  addEventListener() {
    $(".calendar__prev").on("click", (e) => {
      ...
      const { year, month } = this.createTable(
        this.currentYear,
        this.currentMonth
      );
    });
